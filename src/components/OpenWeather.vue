<template>
  <div class="openWeather">
    <ul>
      <li v-for="(s,index) in sel" :key="s+'sel'" @click="changeThis(index)">
         <font-awesome-icon :icon="['fas', 'location-crosshairs']" v-if="index == 'location'" />
         <font-awesome-icon :icon="['fas', 'temperature-half']" v-if="index == 'current'" />
         <font-awesome-icon :icon="['fas', 'thermometer']" v-if="index == 'stat'" />
         <font-awesome-icon :icon="['fas', 'calendar-days']" v-if="index == 'forecast'" />
         <font-awesome-icon :icon="['fas', 'magnifying-glass']" v-if="index == 'search'" />
         <p>{{ index }}</p>
      </li>
    </ul>
    <section v-if="sel.location" class="location">
      <h2>{{ ci.city }}</h2>
      <p>{{ ci.longitude }}, {{ ci.latitude }}</p>
    </section>
    <section class="twoPart" :class="{'hide': !sel.stat && !sel.current}">
      <div class="left" v-if="sel.current" :class="{'single':!sel.stat}">
        <div class="imgCont">
    ...
    <section class="forecast" v-if="sel.forecast"> 
    ...
    <section class="searcher" v-if="sel.search">
...
export default {
  data() {
    return {
      sel: {location: true, current: true, stat: true, forecast: true, search: true},
      myCity: 'Orlando',
      degree:'F',
      originalTemp:null,
      loading: true,
  ...
  mounted() {
    ...
    this.intervalId = setInterval(() => {
      console.log('update', this.myCity)
      this.getCity(this.myCity, API_KEY)
    }, 11500)
  }
}
